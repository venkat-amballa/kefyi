...
class Product(Resource):
    ...
    parser_product.add_argument(
        "name", type=str, required=True, help="Product name, error: {error_msg}"
    )
    parser_product.add_argument(
        "actual_price",
        type=float,
        required=True,
        help="Invalid Actual Amount, error: {error_msg}",
    )
    parser_product.add_argument(
        "wholesale_price",
        type=float,
        required=True,
        help="Invalid Wholsesale Amount, error: {error_msg}",
    )
    parser_product.add_argument(
        "retail_price",
        type=float,
        required=True,
        help="Invalid Retail Amount, error: {error_msg}",
    )
    parser_product.add_argument(
        "quantity", type=int, required=True, help="Invalid Quantity: error: {error_msg}"
    )
    parser_product.add_argument(
        "category", type=str, required=True, help="Category name, error: {error_msg}"
    )
    parser_product.add_argument(
        "store_id", type=int, required=False, help="Store id: error: {error_msg}"
    )
    ...
    def post(self):
        ...
        store = StoreModel.find_by_id(user_id, data.store_id)
        if not store:
            return {
                "status": False,
                "error_code": "INVALID_STORE",
                "message": "No store with the given id",
            }, 400
        ...
        if ProductModel.find_by_name(name):
            return {
                "status": False,
                "error_code": "PRODUCT_NAME_DUPLICATE",
                "message": "There exists an Product with the same name",
            }, 400
